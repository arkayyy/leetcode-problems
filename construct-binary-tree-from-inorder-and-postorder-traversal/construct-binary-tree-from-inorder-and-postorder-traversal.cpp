...
public:
    TreeNode* util(int i1, int i2, int& j, vector<int>& in, vector<int>&post)
    {
        ...
        node->left = util(i1,i-1,j,in,post);//passing the left range of the inorder array from i+1 to i2, to create the left subtree. It returns the root of the leftt subtree
