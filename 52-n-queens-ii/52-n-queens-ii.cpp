...
public:
    ...
    void util(int col, vector<string>&board, vector<vector<string>>&ans,int n, int& cnt)
    {
        if(col==n)
        {ans.push_back(board); ++cnt; return;}
        
        for(int row = 0; row<n; ++row)
            if(isPossible(row,col,board,n))
            {
                board[row][col] = 'Q';
                util(col+1,board,ans,n,cnt);
                board[row][col] = '.';
            }
    }
