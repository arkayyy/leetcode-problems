...
public:
    int widthOfBinaryTree(TreeNode *root)
    {
        ...
        {
            int len = q.size();
            int mmin = q.front().second; // taking minimum index of current level
            int first, last;
            for (int i = 0; i < len; i++)
            { // traversing through the current level

                int idx = q.front().second - mmin; // subtracting the min index of current level so that a 0-based indexing is followed at every level.
