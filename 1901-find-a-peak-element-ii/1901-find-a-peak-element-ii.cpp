...
public:
    vector<int> findPeakGrid(vector<vector<int>>& mat) {
        ...
        while(lo<=hi)
        {
            ...
            cout<<maxRowIdx<<endl;
            if((mid-1<lo || mat[maxRowIdx][mid]>=mat[maxRowIdx][mid-1]) && (mid+1>hi || mat[maxRowIdx][mid]>mat[maxRowIdx][mid+1])) return {maxRowIdx, mid};
            
            if((mid-1<lo || mat[maxRowIdx][mid]>=mat[maxRowIdx][mid-1])) //only greater than left, not right
                lo=mid+1;
