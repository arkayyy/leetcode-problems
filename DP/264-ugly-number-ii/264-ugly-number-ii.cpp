...
//         return arr[n-1];
        ...
        for(int i = 1; i<n;i++)
        {
            //instead of using a priority queue, at every step we store the minimum number that can be produced by multiplying the results as below
            dp[i] = min(dp[p2]*2, min(dp[p3]*3, dp[p5]*5));
            //whichever pointer produces the minimum element we forward that (can be multiple pointers)
            if(dp[i]==dp[p2]*2) p2++;
            if(dp[i]==dp[p3]*3) p3++;
            if(dp[i]==dp[p5]*5) p5++;
        }
